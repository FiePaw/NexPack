...
public class Form {
    ...
    /**
     * Menampilkan form konfirmasi untuk menerapkan atau menghapus pack berdasarkan nama
     * @param requestedPackName Nama resource pack yang akan diproses
     */
    public void showPackConfirmation(String requestedPackName) {
        ...
        // Mencari pack ID berdasarkan nama (pencarian tidak case-sensitive)
        for (Map.Entry<String, ResourcePackManifest> entry : loader.PACKS_INFO.entrySet()) {
            String name = entry.getValue().header().name();
        ...
        if (packId == null) {
            // Pack tidak ditemukan, kirim pesan ke pemain
        ...
        String packDescription = loader.PACKS_INFO.get(packId).header().description();
        
        // Membuat form konfirmasi modal
        ModalForm.Builder form = ModalForm.builder()
                .title(LanguageManager.getLocaleString(lang, "pack.confirm.title"));
        
        // Menyiapkan konten form dengan deskripsi pack jika tersedia
        StringBuilder content = new StringBuilder();
        content.append(String.format(
                LanguageManager.getLocaleString(lang, "pack.confirm.content"),
                currentlyApplied ? 
                    LanguageManager.getLocaleString(lang, "pack.action.remove") : 
                    LanguageManager.getLocaleString(lang, "pack.action.apply"),
                packName
        ));
        
        content.append("\n\n");
        content.append(LanguageManager.getLocaleString(lang, "pack.status")).append(": ");
        content.append(currentlyApplied ? 
                LanguageManager.getLocaleString(lang, "pack.status.applied") : 
                LanguageManager.getLocaleString(lang, "pack.status.not.applied"));
        
        if (packDescription != null && !packDescription.isEmpty()) {
            content.append("\n\n");
            content.append(LanguageManager.getLocaleString(lang, "pack.description")).append(": ");
            content.append(packDescription);
        }
        
        form.content(content.toString());
        form.button1(LanguageManager.getLocaleString(lang, "pack.confirm.apply"));
        form.button2(LanguageManager.getLocaleString(lang, "pack.confirm.cancel"));
        ...
        form.validResultHandler((modalForm, response) -> {
            if (response.clickedButtonId() == 0) { // Apply button clicked
                ...
                if (isApplied) {
                    // Pack sudah diterapkan, jadi hapus
                    ...
                    connection.sendMessage(LanguageManager.getLocaleString(lang, "pack.removed")
                            .replace("%pack%", finalPackName));
                } else {
                    // Pack belum diterapkan, jadi tambahkan
                    playerPacks = new ArrayList<>(PickPack.storage.getPackIds(xuid));
                ...
                // Kirim feedback bahwa operasi dibatalkan
